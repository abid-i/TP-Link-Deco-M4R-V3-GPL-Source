...
#define MINPOLL         12       /* minimum poll interval. std ntpd uses 12 (12: 4096 sec) */
 ...
 */
#define BIGPOLL         15      /* 2^15 sec ~= 4.5 h */
#define MAXPOLL         19      /* maximum poll interval (12: 1.1h, 17: 36.4h 19: 145.6h). std ntpd uses 17 */
...
static void write_uptime(const char* path, char* uptime)
{
	int fd;
	bb_error_msg("ntpd: open file %s", path); 
...
step_time(double offset)
{
	...
	/* record the latest update time, wanglian, 2016-06-16 */
	//bb_error_msg("pid/ntpd file path %s %s", CONFIG_PID_FILE_PATH, CONFIG_NTPD_FILE_PATH);
	write_uptime(CONFIG_NTPD_FILE_PATH "/ntpd.uptime", buf);
...
//		return;                 /* invalid header values */
	...
	/* else: no peer selected, rc = -1: we want to poll more often */
		...
		if (G.initial_poll_complete == 2) {
			update_local_clock(p);
			G.initial_poll_complete = 3;
		}
	}
...
#undef G
	...
	while (!bb_got_signal) {
		...
		for (item = G.ntp_peers; item != NULL; item = item->link) {
			...
			if (quick_mode) {
				nfds = poll(pfd, i, 100);
				for (j = (G.listen_fd > 0) ? 1 : 0; nfds != 0 && j < i; j++) {
					if (pfd[j].revents) {
						nfds--;
						gettime1900d(); /* sets G.cur_time */
						G.initial_poll_complete = 2;
						recv_and_process_peer_pkt(idx2peer[j]);
						if (G.initial_poll_complete == 3) {
							goto normal;
						} else {
							i--;
							if (i >= 0) {
								idx2peer[j] = idx2peer[i];
								pfd[j] = pfd[i];
								j--;
							}
						}
...
normal:
		if (quick_mode && G.initial_poll_complete == 3) {
			G.initial_poll_complete = 0;
			set_new_values(STATE_NSET, 0, 0);
			G.poll_exp = BURSTPOLL;
			G.polladj_count = 0;
			cnt = G.peer_cnt + INITIAL_SAMPLES;
			quick_mode = 0;
			continue;
		}
