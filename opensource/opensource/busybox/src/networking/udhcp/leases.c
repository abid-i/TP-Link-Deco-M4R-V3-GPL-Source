 ...
 * If chaddr == NULL, this is a conflict lease.
 */
struct dyn_lease* FAST_FUNC add_lease(uint32_t state, 
		...
		const char *hostname, int hostname_len, uint8_t is_old_outside_pool)
{
	...
	if (oldest) {
		...
		oldest->pad[0] = state;
        oldest->is_old_outside_pool = is_old_outside_pool;
	}
...
struct dyn_lease* FAST_FUNC find_lease_by_mac(const uint8_t *mac)
{
	...
	for (i = 0; i < server_config.max_leases; i++)
        ...
        {      
            if (false == g_leases[i].is_old_outside_pool)
            {
			    return &g_leases[i];
            }
        }
...
int FAST_FUNC nobody_responds_to_arp(uint32_t nip, const uint8_t *safe_mac)
{
	struct in_addr temp;
	int r;
	...
	temp.s_addr = nip;
	log_notice("%s belongs to someone, reserving it for %u seconds",
		inet_ntoa(temp), (unsigned)server_config.conflict_time);
	add_lease(DHCPNULL, NULL, nip, server_config.conflict_time, NULL, 0, false);
	return 0;
}
