...
#define ONLINE_DEVICE_FILE  "/tmp/online_devlist"
#define MAX_DEVICE_LINE_LEN       1024
...
// if more than N seconds have passed after last use.
	...
	uint32_t self_ip;                /* the ip of the device which run the udhcpd */
	...
	char *lease_file;
	char *pidfile;
	char *deco_host_name;           /* we sholud add static lease item for Deco device at local 
	                                 * network, the host name will be used to judge whether the Deco
	                                 * device */ 
	char *notify_file;              /* what to run whenever leases are written */
	char *sname;                    /* bootp server name */
	char *boot_file;                /* bootp boot file option */
	struct static_lease *static_leases; /* List of ip/mac pairs to assign static leases */
	char *filtered_host_name;       /* Don't respond to the DHCP message which containing the host 
	                                 * name */
} FIX_ALIASING;
...
struct dyn_lease {
	...
	char hostname[32];
	uint8_t pad[2];
    uint8_t is_old_outside_pool;
	/* total size is a multiply of 4 */
} PACKED;

extern struct dyn_lease *g_leases;

struct dyn_lease *add_lease(uint32_t state, 
		const uint8_t *chaddr, uint32_t yiaddr,
		leasetime_t leasetime,
		const char *hostname, int hostname_len, uint8_t is_old_outside_pool
		) FAST_FUNC;
...
int nobody_responds_to_arp(uint32_t nip, const uint8_t *safe_mac) FAST_FUNC;
