...
#define s (pfd[0].fd)           /* socket */
	...
	timeout_ms = 500;	/* modified from 2000 to 500 by hx */
	do {
		...
		if (r) {
            ...
            else if (r >= ARP_MSG_SIZE
			 && arp.operation == htons(ARPOP_REQUEST)
			 /* don't check it: Linux doesn't return proper tHaddr (fixed in 2.6.24?) */
			 /* && memcmp(arp.tHaddr, from_mac, 6) == 0 */
			 && *(aliased_uint32_t*)arp.sInaddr == *(aliased_uint32_t*)arp.tInaddr
			 && *(aliased_uint32_t*)arp.sInaddr == test_nip
			) {
			    bb_info_msg("receive Garp, safe_mac:%02x:%02x:%02x:%02x:%02x:%02x; sHaddr:%02x:%02x:%02x:%02x:%02x:%02x",
                   safe_mac[0], safe_mac[1], safe_mac[2], safe_mac[3], safe_mac[4], safe_mac[5], 
		 ...
		 */
	} while ((unsigned)timeout_ms <= 500);	/* modified from 2000 to 500 by hx */

 ret:
	close(s);
	log1("%srp reply received for this address", rv ? "No a" : "A");
    bb_info_msg("%srp reply received for this address", rv ? "No a" : "A");
	return rv;
}
