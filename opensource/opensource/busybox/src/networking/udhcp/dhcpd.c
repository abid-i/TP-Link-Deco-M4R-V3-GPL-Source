...
static int get_product_name()
{
    ...
    log_notice("model_name:%s", model_name);
...
static int get_wan_hostname()
{
    ...
    log_notice("wan_host_name:%s", wan_host_name);
	return 0;
}
		...
		uint8_t *requested_ip_opt)
{
	...
	if (!static_lease_nip) {
		...
		lease = add_lease(DHCPOFFER, packet.chaddr, packet.yiaddr,
				...
				p_host_name,
				p_host_name ? (unsigned char)p_host_name[OPT_LEN - OPT_DATA] : 0, false
		);
		if (!lease) {
			bb_error_msg("no free IP addresses. OFFER abandoned");
			return;
	...
	log_notice("Sending OFFER of %s", inet_ntoa(addr));
...
static NOINLINE void send_ACK(struct dhcp_packet *oldpacket, uint32_t yiaddr)
{
	...
	addr.s_addr = yiaddr;
	log_notice("Sending ACK to %s", inet_ntoa(addr));
	send_packet(&packet, /*force_bcast:*/ 0);
...
int udhcpd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	log_notice("%s (v"BB_VER") started", applet_name);
 ...
 continue_with_autotime:
	...
	while (1) { /* loop until universe collapses */
		...
		case SIGUSR1:
			log_notice("Received SIGUSR1");
			write_leases();
			/* why not just reset the timeout, eh */
			goto continue_with_autotime;
#ifdef __TP_DHCP__
		case SIGUSR2:
			log_notice("Received SIGUSR2,%d %s %s",argc,argv[0],argv[1]);
		...
		if (static_lease_nip)
		{
			if ((state[0] == DHCPDISCOVER) || (state[0] == DHCPREQUEST))
			{
			    if (!nobody_responds_to_arp(static_lease_nip, packet.chaddr) || 
				    (static_lease_nip == server_config.self_ip))
				{
					static_lease_nip = 0;
				}
			}
		...
		/* end added */
		if (static_lease_nip) {
			log_notice("Found static lease: %x", static_lease_nip);
...
#if 0			
			/*新设备获取IP时会在Offer阶段进行arp检查；在此处仅需对在lease表中的IP进行配置
			  检查来规避与静态IP发生冲突的问题；lease->pad[0] == DHCPACK 表明此IP是以完成
			...
			if (lease && ((state[0] == DHCPDISCOVER) || (state[0] == DHCPREQUEST)))
			{
                if (is_ip_conflict(lease->lease_nip, packet.chaddr) 
			        || (lease->lease_nip == server_config.self_ip))
				{
					lease = NULL;
				}
			}					
