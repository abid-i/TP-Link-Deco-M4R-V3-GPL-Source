...
/* state variable used under mixbroadcast mode */
...
static void add_client_options(struct dhcp_packet *packet)
{
	...
	/* Request broadcast replies if we have no IP addr and under mixbroadcast mode, 
	 * it also depends on the state of usingBroadcast */
 ...
 */
	...
	log_notice("Sending select for %s...", inet_ntoa(addr));
	return raw_bcast_from_client_config_ifindex(&packet, INADDR_ANY);
}
...
skip_udp_sum_check:
	...
	if (client_config.mode[0] == 'i' || client_config.mode[0] == 'e') {
		char mac[HOSTNAME_MAX_LEN + 1] = {0};
		uint8_t *sname =  (uint8_t *)udhcp_get_option(&packet.data, DHCP_HOST_NAME);
		uint8_t host_len = 0;
		int ret = 0;

		if (sname)
		{
			host_len= sname[-OPT_DATA + OPT_LEN];
		}

		if (host_len > HOSTNAME_MAX_LEN)
		{
			return -2;
		}

		/* make sure mac is end of '\0' */
		ret = deco_get_mac_from_sname(sname, mac, host_len);
		if (ret < 0) {		
			if (client_config.mode[0] == 'i') {
				log_notice("Packet UNEXPECTEDLY NOT from group devices, ignoring");
				return -2;
			}
		}
		else {
			...
			if (res == NULL) {
				...
				if (client_config.mode[0] == 'e') {
					log_notice("Packet UNEXPECTEDLY from group devices, ignoring");
					return -2;
				}
			}
		}
...
//usage:     "\n	-M,--mixbroadcast	Request unicast or broadcast replies when necessary"
...
int udhcpc_main(int argc UNUSED_PARAM, char **argv)
{
	...
	opt = getopt32(argv, "CV:H:h:F:i:unp:qQRr:s:T:t:SA:O:ox:fBMg:m:"
			USE_FOR_MMU("b")
...
//		bb_info_msg("enter loop, state=%d listen_mode=%d", state, listen_mode);
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				if (!discover_retries)
				{
					if (packet_num > 0 && 0 == packet_num % 3)
					{
			...
			case REQUESTING:
				if (opt & OPT_Q) {
					...
					log_notice("DHCP offer received, exit");
					goto ret0;
				}
 ...
 * the configuration process..." */
				...
				temp_addr_c.s_addr = packet.ciaddr;
				temp_addr.s_addr = packet.yiaddr;

				if (temp_addr_c.s_addr && (temp_addr.s_addr != temp_addr_c.s_addr)) {
					/* client ip given by server is abnormal, enter init state */
					bb_info_msg("abnormal client address, entering init state");
					udhcp_run_script(NULL, "deconfig");
