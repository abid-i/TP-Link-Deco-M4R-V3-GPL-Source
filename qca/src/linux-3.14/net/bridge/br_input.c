...
int br_pass_frame_up(struct sk_buff *skb)
{
	...
	indev = skb->dev;
	skb->dev = brdev;
	if (0 == strncmp(indev->name, "eth0", strlen("eth0")))
	{
		skb->dev->priv_flags |= IFF_INDICATE_DEV0;
		skb->dev->priv_flags &= ~IFF_INDICATE_DEV1;
	}
	else if(0 == strncmp(indev->name, "eth1", strlen("eth1")))
	{
		skb->dev->priv_flags |= IFF_INDICATE_DEV1;
		skb->dev->priv_flags &= ~IFF_INDICATE_DEV0;
	}
...
#ifdef CONFIG_BRIDGE_DELIVER_CONTROL
    if(!br_should_deliver_packet(br, skb, vid))
    {
        goto drop;
    }
#endif    
...
#ifdef CONFIG_BRIDGE_DELIVER_CONTROL
    br_fdb_update_mapped_hwaddr(br, p, skb, vid, false);

    if (eth_hdr(skb)->h_proto == htons(ETH_P_IP))
    {
        br_fdb_update_ip(br, p, eth_hdr(skb)->h_source, ip_hdr(skb)->saddr, vid);
    }
#endif
