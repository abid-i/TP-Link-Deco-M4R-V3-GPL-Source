...
#endif
#ifdef CONFIG_BRIDGE_DELIVER_CONTROL
static ssize_t bridge_group_matrix_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	int i,j, ret,len=0;
	...
	ret=sprintf(pos,"==================group define results====================\n");
	len+=ret;
	pos+=ret;

	for(i=0;i<BRIDGE_GROUP_MAX_TYPE;i++)
	{
	    for (j =0; j < BRIDGE_GROUP_DEV_NUM; j++)
	    {
	        if (br->bridge_group_matrix[i][j][0] != '\0')	
	        {
		        ret=sprintf(pos,"%s",br->bridge_group_matrix[i][j]);
		        len+=ret;
		        pos+=ret;
	        }
	        ret=sprintf(pos,"%s", " ");
...
static ssize_t bridge_group_matrix_store(struct device *d,
				struct device_attribute *attr,
				const char *buf, size_t len)
{
	struct net_bridge *br = to_bridge(d);
	char name[BRIDGE_DELIVER_GROUP_MAX_SIZE + 1]={0};
	unsigned long max_length=len;
	int i=0,devindex=0,subindex=0,namelen=0;

	if (!ns_capable(dev_net(br->dev)->user_ns, CAP_NET_ADMIN))
		return -EPERM;

	if(max_length>BRIDGE_DELIVER_GROUP_MAX_SIZE)
		max_length=BRIDGE_DELIVER_GROUP_MAX_SIZE;

	memcpy(name, buf, max_length);
	name[max_length] = 0;
	...
	while(i<=max_length)
	{
		...
		if(name[i]=='\0'||name[i]=='\n')
		{
			br->bridge_group_matrix[devindex++][subindex][namelen]='\0';
			break;
		}
		else if(name[i]==':')
		{
		...
		{			
			br->bridge_group_matrix[devindex][subindex][namelen++]=name[i];
			i++;
		}
	}    
...
static DEVICE_ATTR_RW(bridge_group_matrix);

static ssize_t bridge_hyfi_extend_enable_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%d\n", br->bridge_hyfi_extend_enable);
}

static int set_bridge_hyfi_extend_enable(struct net_bridge *br, unsigned long val)
{
	br->bridge_hyfi_extend_enable = val;
	return 0;
}

static ssize_t bridge_hyfi_extend_enable_store(
	struct device *d, struct device_attribute *attr, const char *buf,
	size_t len)
{
	return store_bridge_parm(d, buf, len, set_bridge_hyfi_extend_enable);
}
static DEVICE_ATTR_RW(bridge_hyfi_extend_enable);


static ssize_t bridge_name_matrix_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	int i,j, ret,len=0;
	...
	{
        for (j =0; j < BRIDGE_DELIVER_SUBDEV_NUM; j++)
	    {
	        if (br->bridge_name_matrix[i][j][0] != '\0')	
	        {
		        ret=sprintf(pos,"%s",br->bridge_name_matrix[i][j]);
		        len+=ret;
		        pos+=ret;
	        }
            ret=sprintf(pos,"%s", " ");
...
static ssize_t bridge_name_matrix_store(struct device *d,
				struct device_attribute *attr,
				const char *buf, size_t len)
{
	struct net_bridge *br = to_bridge(d);
	char name[BRIDGE_DELIVER_DEV_MAX_SIZE + 1]={0};
	unsigned long max_length=len;
	int i=0,devindex=0,subindex=0,namelen=0;
	...
	while(i<=max_length)
	{
		...
		if(name[i]=='\0'||name[i]=='\n')
		{
			br->bridge_name_matrix[devindex++][subindex][namelen]='\0';
			break;
		}
		else if(name[i]==':')
		{
		...
		{			
			br->bridge_name_matrix[devindex][subindex][namelen++]=name[i];
			i++;
		}
	}    
...
static DEVICE_ATTR_RW(bridge_name_matrix);

static ssize_t bridge_deliver_enable_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%d\n", br->bridge_deliver_enable);
}

static int set_bridge_deliver_enable(struct net_bridge *br, unsigned long val)
{
	br->bridge_deliver_enable = val;
    br->free_bridage_fdb.is_enabled = 0;
	return 0;
}

static ssize_t bridge_deliver_enable_store(
	struct device *d, struct device_attribute *attr, const char *buf,
	size_t len)
{
	return store_bridge_parm(d, buf, len, set_bridge_deliver_enable);
}
static DEVICE_ATTR_RW(bridge_deliver_enable);

static ssize_t bridge_deliver_link_state_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%d\n", br->bridge_deliver_link_state);
}

static ssize_t bridge_deliver_link_state_store(
	struct device *d, struct device_attribute *attr, const char *buf,
	size_t len)
{
	return store_bridge_parm(d, buf, len, br_set_link_state);
}
static DEVICE_ATTR_RW(bridge_deliver_link_state);


static ssize_t bridge_deliver_eth_to_2g_enable_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
...
static ssize_t bridge_deliver_eth_to_2g_enable_store(
	struct device *d, struct device_attribute *attr, const char *buf,
	size_t len)
{
	return store_bridge_parm(d, buf, len, br_set_eth_to_2g_enable);
}
static DEVICE_ATTR_RW(bridge_deliver_eth_to_2g_enable);

static ssize_t bridge_deliver_wireless_repeater_3addrees_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%d\n", br->bridge_deliver_wireless_repeater_3addrees);
}

static int set_bridge_deliver_wireless_repeater_3addrees(struct net_bridge *br, unsigned long val)
{
	br->bridge_deliver_wireless_repeater_3addrees = val;
	return 0;
}

static ssize_t bridge_deliver_wireless_repeater_3addrees_store(
	struct device *d, struct device_attribute *attr, const char *buf,
	size_t len)
{
	return store_bridge_parm(d, buf, len, set_bridge_deliver_wireless_repeater_3addrees);
}
static DEVICE_ATTR_RW(bridge_deliver_wireless_repeater_3addrees);

static ssize_t bridge_deliver_has_same_front_dut_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
    br->bridge_deliver_has_same_front_dut= br->free_bridage_fdb.is_same_front_dut;
	return sprintf(buf, "%d\n", br->bridge_deliver_has_same_front_dut);
}

static int set_bridge_deliver_has_same_front_dut(struct net_bridge *br, unsigned long val)
{
	br->bridge_deliver_has_same_front_dut = val;
    br->free_bridage_fdb.is_same_front_dut= br->bridge_deliver_has_same_front_dut;
	return 0;
}

static ssize_t bridge_deliver_has_same_front_dut_store(
	struct device *d, struct device_attribute *attr, const char *buf,
	size_t len)
{
	return store_bridge_parm(d, buf, len, set_bridge_deliver_has_same_front_dut);
}
static DEVICE_ATTR_RW(bridge_deliver_has_same_front_dut);

static ssize_t bridge_deliver_dbg_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%d\n", br->bridge_deliver_dbg);
}

static int set_bridge_deliver_dbg(struct net_bridge *br, unsigned long val)
{
	br->bridge_deliver_dbg = val;
	return 0;
}

static ssize_t bridge_deliver_dbg_store(
	struct device *d, struct device_attribute *attr, const char *buf,
	size_t len)
{
	return store_bridge_parm(d, buf, len, set_bridge_deliver_dbg);
}
static DEVICE_ATTR_RW(bridge_deliver_dbg);

static ssize_t bridge_deliver_psr_alias_rule_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%d\n", br->bridge_deliver_psr_alias_rule);
}

static int set_bridge_deliver_psr_alias_rule(struct net_bridge *br, unsigned long val)
{
	br->bridge_deliver_psr_alias_rule = val;
	return 0;
}

static ssize_t bridge_deliver_psr_alias_rule_store(
	struct device *d, struct device_attribute *attr, const char *buf,
	size_t len)
{
	return store_bridge_parm(d, buf, len, set_bridge_deliver_psr_alias_rule);
}
static DEVICE_ATTR_RW(bridge_deliver_psr_alias_rule);

static ssize_t bridge_deliver_filter_table_show(
	struct device *d, struct device_attribute *attr, char *buf)
{
    struct net_bridge *br = to_bridge(d);
	int i,j,ret,len=0;
...
//filter_table_2g_5g	
	...
	for(i=0;i<7;i++)
	{
		...
		for(j=0;j<7;j++)
		{
			ret=sprintf(pos,"\t%d",br->free_bridage_fdb.filter_table_2g_5g[i][j]);
			len+=ret;
			pos+=ret;
		}
		ret=sprintf(pos,"\n");
...
static DEVICE_ATTR_RO(bridge_deliver_filter_table);
...
#endif
#ifdef CONFIG_BRIDGE_DELIVER_CONTROL
    &dev_attr_bridge_group_matrix.attr,
    &dev_attr_bridge_name_matrix.attr,
    &dev_attr_bridge_hyfi_extend_enable.attr,
    &dev_attr_bridge_deliver_enable.attr,
    &dev_attr_bridge_deliver_link_state.attr,
    &dev_attr_bridge_deliver_eth_to_2g_enable.attr,
    &dev_attr_bridge_deliver_wireless_repeater_3addrees.attr,
    &dev_attr_bridge_deliver_has_same_front_dut.attr,
    &dev_attr_bridge_deliver_dbg.attr,
    &dev_attr_bridge_deliver_psr_alias_rule.attr, 
    &dev_attr_bridge_deliver_filter_table.attr,
#endif
#ifdef CONFIG_BRIDGE_NETFILTER
