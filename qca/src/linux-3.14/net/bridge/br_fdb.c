					       ...
					       __u16 vid)
{
	...
	if (fdb) {
		...
		fdb->ip = 0;
#endif	/* CONFIG_BRIDGE_DELIVER_CONTROL */
...
#ifdef CONFIG_BRIDGE_DELIVER_CONTROL
void br_fdb_update_ip(struct net_bridge *br, struct net_bridge_port *source,
                   const unsigned char *addr, const unsigned int ip, u16 vid)
{
    struct hlist_head *head = &br->hash[br_mac_hash(addr, vid)];
    struct net_bridge_fdb_entry *fdb;
...
#define BACKEND_CARRIER_TIMEOUT 500
int br_fdb_check_ring_packet(struct sk_buff *skb, struct tp_db_fre_bridge_fdb_entry *p, u16 vid)
{
    struct net_bridge *br = p->br0_bri_dev;
    unsigned char *saddr = eth_hdr(skb)->h_source;
    // unsigned char *daddr = eth_hdr(skb)->h_dest;
    struct hlist_head *head = &br->hash[br_mac_hash(saddr, vid)];
    struct net_bridge_fdb_entry *sfdb = NULL;
    unsigned long freshed  = 0;
    ...
    spin_lock_bh(&br->hash_lock);
    sfdb = fdb_find(head, saddr, vid);
    // dfdb = fdb_find(head,daddr);

    if (likely(sfdb && sfdb->dst)) {
        if (sfdb->is_local) {
                //ring packet
                p->is_same_front_dut = DUAL_REPEATER_HAS_SAME_FROMT_DUT;
                ret = 0;
        }
        else if (sfdb->dst->dev->br_port_index <= MAX_BACK_DEV_BR_INDEX_VALUE) {
            freshed = sfdb->updated + msecs_to_jiffies(BACKEND_CARRIER_TIMEOUT);
            if (time_before(jiffies, freshed))
            {
                //ring packet
                p->is_same_front_dut = DUAL_REPEATER_HAS_SAME_FROMT_DUT;
                ret = 0;
    ...
    else if (ether_addr_equal(br->dev->dev_addr, saddr))
    {
        p->is_same_front_dut = DUAL_REPEATER_HAS_SAME_FROMT_DUT;
        ret = 0;        
    }
