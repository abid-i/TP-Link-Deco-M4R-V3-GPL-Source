		     ...
		     struct packet_type *pt, struct net_device *orig_dev)
{
	...
	if (0 != memcmp(eth_hdr(skb)->h_dest, dev->dev_addr, ETH_ALEN))
	{
		printk("In pppoe_rcv, drop invalid destination: destMac=%02x:%02x:%02x:%02x:%02x:%02x\n",
...

{
	...
	if (po) {
		...
		if (0 != memcmp(eth_hdr(skb)->h_dest, dev->dev_addr, ETH_ALEN))
		{
			printk("In pppoe_disc_rcv, drop invalid destination:: destMac=%02x:%02x:%02x:%02x:%02x:%02x\n",
					eth_hdr(skb)->h_dest[0], eth_hdr(skb)->h_dest[1],
					eth_hdr(skb)->h_dest[2], eth_hdr(skb)->h_dest[3],
					eth_hdr(skb)->h_dest[4], eth_hdr(skb)->h_dest[5]);
			goto abort;
...
static int pppoe_release(struct socket *sock)
{
	...
	if (po->pppoe_pa.sid) {
		rcu_read_lock();
		pppoe_destroy_method = rcu_dereference(pn->pppoe_destroy_method);
		destroy_method_arg = rcu_dereference(pn->destroy_method_arg);
		  ...
		  int sockaddr_len, int flags)
{
	...
	if (stage_session(po->pppoe_pa.sid)) {
		 ...
		 */
		write_lock_bh(&pn->hash_lock);
		rcu_assign_pointer(pn->pppoe_destroy_method, NULL);
		rcu_assign_pointer(pn->destroy_method_arg, NULL);
		write_unlock_bh(&pn->hash_lock);
	}
...
static bool pppoe_register_destroy_method(struct ppp_channel *chan, ppp_channel_destroy_method_t method, void *destroy_method_arg)
{
	...
	rcu_assign_pointer(pn->pppoe_destroy_method, method);
	rcu_assign_pointer(pn->destroy_method_arg, destroy_method_arg);
	write_unlock_bh(&pn->hash_lock);
