...
/* add by wanghao */
struct timer_list wanDevLinkStatusTimer;
/* add end */
...
/* add by wanghao */
extern int (*fucnPtr_fal_port_link_status_get)(unsigned int dev_id, unsigned int port_id, unsigned int * status);
static struct work_struct work;
static work_func_t linkStatusCheckCallback = NULL;
static int linkStatusCheckLock = 0;
#define printWar(fmt, args...) printk("\033[4m[ %s ] %03d: "fmt"\033[0m", __FUNCTION__, __LINE__, ##args)
static void linkStatusCheck(void)
{
	struct edma_adapter *adapter = netdev_priv(netdev[EDMA_LAN]);
	int32_t status = 0;
	
	fucnPtr_fal_port_link_status_get(0, 4, &status);
	
	if (adapter->link_state == __EDMA_LINKDOWN && status == 1)//linkup
	{
		//printWar("eth1 link up!!!\n");
		adapter->link_state = __EDMA_LINKUP;
		netif_carrier_on(netdev[EDMA_LAN]);
		if (netif_running(netdev[EDMA_LAN]))
			netif_tx_wake_all_queues(netdev[EDMA_LAN]);
	}
	else if (adapter->link_state == __EDMA_LINKUP && status == 0)//linkdown
	{
		//printWar("eth1 link down!!!\n");
		adapter->link_state = __EDMA_LINKDOWN;
		netif_carrier_off(netdev[EDMA_LAN]);
		netif_tx_stop_all_queues(netdev[EDMA_LAN]);
	}
	
	linkStatusCheckLock = 0;
}

static void wanDevLinkStatusUpdate(unsigned long unused)
{
	if (fucnPtr_fal_port_link_status_get != NULL)
	{
		if (!linkStatusCheckLock)
		{
			linkStatusCheckLock = 1;
			INIT_WORK(&work, linkStatusCheckCallback);
			schedule_work(&work);
		}
	}
	else
	{
		printWar("fucnPtr_fal_port_link_status_get is NULL!\n");
	}

	mod_timer(&wanDevLinkStatusTimer, jiffies + 2*HZ);
}
/* add end */
...
static int edma_axi_probe(struct platform_device *pdev)
{
	...
	/* add by wanghao */
	adapter[EDMA_LAN]->link_state = __EDMA_LINKUP;
	
	linkStatusCheckCallback = (work_func_t)linkStatusCheck;
	
	init_timer(&wanDevLinkStatusTimer);
	wanDevLinkStatusTimer.expires = jiffies + 2*HZ;
	wanDevLinkStatusTimer.function = wanDevLinkStatusUpdate;
	add_timer(&wanDevLinkStatusTimer);
	/* add end */
