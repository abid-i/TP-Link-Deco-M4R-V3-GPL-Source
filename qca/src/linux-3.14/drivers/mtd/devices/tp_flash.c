...
tp_flash_ioctl(struct file *file,  unsigned int cmd, unsigned long arg)
{
	...
	//printk(KERN_ERR"ADDR=%08x,\thasHead=%u", addr, hasHead);
	//pr_err("ADDR=%08x,\thasHead=%u", addr, hasHead);

    if (NULL == (mtd = mtd_for_flash_chardev)) {
        printk(KERN_ERR"flash_device_mtd is NULL!\n");
		pr_err("flash_device_mtd is NULL!\n");
        goto wrong;
    }
    if (rwBuf == NULL)
    {
        printk(KERN_ERR"rw_buf error!\n");
		pr_err("rw_buf error!\n");
        goto wrong;
    }
    if (_IOC_TYPE(cmd) != TP_IO_MAGIC)
    {
		...
		pr_err("cmd type error!\n");
        goto wrong;
    }
    if (_IOC_NR(cmd) > TP_IOC_MAXNR)
    {
    	printk(KERN_ERR"cmd NR error!\n");
        pr_err("cmd NR error!\n");
        goto wrong;
    }
...
static int tp_flash_open (struct inode *inode, struct file *filp)
{
    ...
    if ((filp->f_mode & 2) && (minor & 1)) {
		...
		pr_err("You can't open the RO devices RW!\n");
        return -EACCES;
    }
	//pr_err("Open tp flash successfully\n");
    return 0;
}
...
static int __init flash_chrdev_init (void)
{
    ...
    int ret = 0;
    int err;
    int tp_flash_major = flash_major;
    int tp_flash_minor = flash_minor;

    printk(KERN_ERR"flash_chrdev : flash_chrdev_init \n");
	pr_err("flash_chrdev : flash_chrdev_init \n");

    if (tp_flash_major) {
        dev = MKDEV(tp_flash_major, tp_flash_minor);
        ret = register_chrdev_region(dev, 1, "flash_chrdev");
    }
    else {
        ret = alloc_chrdev_region(&dev, tp_flash_minor, 1, "flash_chrdev");
        tp_flash_major = MAJOR(dev);
    }

    if (ret < 0) {
        printk(KERN_WARNING "flash_chrdev : can`t get major %d\n", tp_flash_major);
		pr_err("flash_chrdev : can`t get major %d\n", tp_flash_major);
        goto fail;
    }
    ...
    if (err) 
   	{
		...
		pr_err("error %d adding flash_chrdev ", err);
    }
