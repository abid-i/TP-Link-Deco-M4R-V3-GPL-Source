...
#ifdef NM_DUAL_IMAGE
    base = NM_PTN_TABLE_BASE_0;
#else
    base = NM_PTN_TABLE_BASE;
#endif

    nm_lib_initPtnStruct();
    memset(&ptnTable, 0, sizeof(ptnTable));

    NM_DEBUG("NM_PTN_TABLE_BASE = 0x%x",  base);
    
    NM_INFO("Reading Partition Table from NVRAM ... ");
    ret = nm_lib_readPtnFromNvram((char *)base, (char *)&ptnTable, NM_PTN_TABLE_SIZE);
    if (ret < 0)
    {
        NM_INFO("FAILED\r\n");
        goto failed;
