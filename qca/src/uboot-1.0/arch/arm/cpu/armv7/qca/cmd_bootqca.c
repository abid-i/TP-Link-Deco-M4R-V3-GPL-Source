 ...
 */
static int set_fs_bootargs(int *fs_on_nand, int idx)
{
	char *bootargs;
	char nor_rootfs[64];
...
#define nand_rootfs	"ubi.mtd=" QCA_ROOT_FS_PART_NAME " root=mtd:ubi_rootfs rootfstype=squashfs"
	...
	if (sfi->flash_type == SMEM_BOOT_SPI_FLASH) {
		if (((sfi->rootfs[idx].offset == 0xBAD0FF5E) &&
...
#endif
			snprintf(nor_rootfs, sizeof(nor_rootfs),
				 "root=mtd:%d:rootfs rootfstype=squashfs", idx);
			bootargs = nor_rootfs;
			*fs_on_nand = 0;
			if (getenv("fsbootargs") == NULL)
				setenv("fsbootargs", bootargs);
...
static void fs_bootargs_checkpoint(int save)
{
    static char *bootargs = NULL;
    static char *fsbootargs = NULL;

    if (save)
    {
        bootargs = getenv("bootargs");
        fsbootargs = getenv("fsbootargs");
    }
    else
    {
        setenv("bootargs", bootargs);
        setenv("fsbootargs", fsbootargs);
    }
}
                             ...
                             int idx)
{
...
#endif
	if ((ret = set_fs_bootargs(&ipq_fs_on_nand, idx)))
		return ret;
	...
	if (ipq_fs_on_nand) {
		if (sfi->rootfs[idx].offset == 0xBAD0FF5E) {
			sfi->rootfs[idx].offset = 0;
			sfi->rootfs[idx].size = IPQ_NAND_ROOTFS_SIZE;
		}
		/*
		...
		snprintf(runcmd, sizeof(runcmd),
			...
			gboard_param->spi_nand_available,
			gboard_param->spi_nand_available,
			sfi->rootfs[idx].size, sfi->rootfs[idx].offset,
			request);
		...
		snprintf(runcmd, sizeof(runcmd),
			...
			request, sfi->hlos[idx].offset, sfi->hlos[idx].size);
		...
		kernel_img_info.kernel_load_size =  sfi->hlos[idx].size;
	}
	...
	if (sfi->flash_type == SMEM_BOOT_NAND_FLASH) {
		...
		if ((sfi->rootfs[idx].offset == 0xBAD0FF5E) ||
			...
			snprintf(runcmd, sizeof(runcmd),
				...
				CONFIG_SYS_LOAD_ADDR, (uint)sfi->hlos[idx].offset, (uint)sfi->hlos[idx].size);
		}
#ifdef CONFIG_QCA_MMC
	} else if ((sfi->flash_type == SMEM_BOOT_MMC_FLASH) || (gboard_param->nor_emmc_available == 1)) {
		if (debug) {
...
check_fw_gpio(void)
{
	unsigned int *addr = (unsigned int *)GPIO_IN_OUT_ADDR(CONFIG_FWRECOVERY_KEY);
	int val = readl(addr);
	return !(val & 0x1);
}
...
static void
set_fw_led(int state)
{
	uchar bytes[] = {0 /* unused */, 0x3f, 0, 0, 0};
    uchar color[] = {0, 0, 0};
    uchar control = 0b01010101;
	int i;
	switch (state)
	{
	case FW_LED_START:
		run_command("debugFy 3\n", 0);
		return;
	...
	/* Skip 0x0 */
	for (i = 1; i < sizeof(bytes)/sizeof(bytes[0]); i++)
	{
		i2c_write(0x32, i, 1, &bytes[i], 1);
	}

	for (i = 0; i < sizeof(color)/sizeof(color[0]); i++)
    {
		i2c_write(0x30, 8-i, 1, &color[i], 1);
    }
    i2c_write(0x30, 4, 1, &control, 1);
}

static int up_file(void)
{
	int count = 8;
	set_fw_led(FW_LED_START);

		if (run_command("debugFy 1\n", 0)
			!= CMD_RET_SUCCESS)
		{
			set_fw_led(FW_LED_FAILURE);
			return CMD_RET_FAILURE;
		}
	...
	return CMD_RET_FAILURE;
#else
	while (run_command("fwrecovery 0x84000000 " CONFIG_FWRECOVERY_FILE "\n", 0)
		!= CMD_RET_SUCCESS)
	{
		if (count > 0)
		{
...
static int do_bootipq(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (ret == 0 && buf == 1) {
        do_boot = do_boot_signedimg;
	} else if (ret == 0 || ret == -EOPNOTSUPP) {
        do_boot = do_boot_unsignedimg;
	}

    if (do_boot)
    {
		char *boot_idx = getenv("boot_idx");
		if (boot_idx != NULL)
		{
			unsigned long idx = simple_strtoul(boot_idx, NULL, 10);
			return do_boot(cmdtp, flag, argc, argv, idx);
		}

        fs_bootargs_checkpoint(1);
        ret = do_boot(cmdtp, flag, argc, argv, 1);
        if (CMD_RET_FAILURE == ret)
        {
            fs_bootargs_checkpoint(0);
            ret = do_boot(cmdtp, flag, argc, argv, 0);
        }
